...
function getNextPrayer(prayers) {
  ...
  return prayers[0]; // wrap to Subuh next day
}
...
function startShalatCountdown(timeStr) {
  ...
  window._shalatTimer = setInterval(() => {
    if (!document.getElementById('shalat-countdown')) {
...
async function renderQiblat(lat, lon) {
  ...
  box.innerHTML = `
    ...
    <div class="qiblat-kaaba-card">
      <div class="qiblat-kaaba-left">
        ...
        <div>
          <p style="font-size:14px;font-weight:700;color:var(--text)">Ka'bah, Masjidil Haram</p>
          <p style="font-size:12px;color:var(--text-muted)">Mekah, Arab Saudi</p>
          <p style="font-size:11px;color:var(--text-muted);margin-top:2px">21.4225° N, 39.8262° E</p>
        </div>
      </div>
    </div>
  ...
  // Animate compass needle with device orientation if available
