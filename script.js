...
async function downloadFullSurah(surahNum, qariKey) {
  ...
  showToastDl(`⬇ Menyiapkan unduhan Full Surah ${s.latin} (${total} ayat)...`);

  for (let i = 1; i <= total; i++) {
    const url   = buildAudioUrl(surahNum, i, qKey);
    const fname = `${s.latin}_${pad(surahNum)}${pad(i)}_${q.name.replace(/\s/g,'_')}.mp3`;
    const a     = document.createElement('a');
    a.href      = url;
    a.rel       = 'noopener';
    a.download  = fname;
    ...
    if (i % 5 === 0 || i === total) {
      showToastDl(`⬇ Mengunduh ${s.latin} · Ayat ${i}/${total}`);
    }
    // Jeda kecil antar unduhan agar browser tidak memblokir multi-download
    await new Promise(r => setTimeout(r, 500));
  }

  _dlSurahRunning = false;
  showToastDl(`✅ Selesai! ${s.latin} Full (${total} ayat) · ${q.name} terunduh`);
}
